<?php
    ...
    class SabreAMF_Client {
         ...
         * @return $this
         */
        public function addHTTPCookie($cookie)
        {
         ...
         * @return mixed 
         */
        public function sendRequests($servicePath,$data) {
         ...
         * @return mixed 
         */
        public function sendRequest($servicePath,$data,$response='/1',$is_encode=false) {

            $this->amfRequest = new SabreAMF_Message();
            $this->amfRequest->setEncoding($this->encoding);
            $this->amfOutputStream = new SabreAMF_OutputStream();
            ...
            curl_setopt($ch, CURLOPT_USE_SSL,0);
            curl_setopt($ch, CURLOPT_HEADER,1);
            //curl_setopt($ch, CURLOPT_ENCODING,"ISO-8859-1");

            if($is_encode) {
                $encoded_data = mb_convert_encoding($this->amfOutputStream->getRawData(), "ISO-8859-1", "UTF-8");
            } else {
                $encoded_data = $this->amfOutputStream->getRawData();
            }
            ...
            preg_match_all('/Set-Cookie: JSESSIONID=\s*([^ ]*);/mi', $result_header, $matchesh);

            //var_dump($matchesh);
            
            if(count($matchesh[1]) > 0) {

                $return_cookie = $matchesh[1][0];

                $return_cookie = str_replace("\r", "", $return_cookie);
                $return_cookie = str_replace("\n", "", $return_cookie);
                
                //var_dump($returnh);

                $this->addHTTPCookie("JSESSIONID=".$return_cookie);
            }

            $this->amfInputStream = new SabreAMF_InputStream($result_body);
            $this->amfResponse = new SabreAMF_Message();
            $this->amfResponse->deserialize($this->amfInputStream);
            ...
            foreach($this->amfResponse->getBodies() as $body) {
                ...
                if (strpos($body['target'], $response)===0) return $body['data'] ;
